...
export function WalkthroughMediaRecorder({
  ...
  const runAnalyze = async (body: Record<string, unknown>) => {
    ...
    try {
      ...
      let frameStoragePaths: string[] | null = null;

      const jpegFrames: string[] = [];
      if (mode === "video") {
        for (const f of framesRef.current) {
          if (typeof f === "string" && f.length > 0) jpegFrames.push(f);
        }
      } else if (mode === "audiovideo") {
        for (const b of avBurstsRef.current) {
      ...
      if (jpegFrames.length > 0) {
        try {
          frameStoragePaths = await uploadWalkthroughFrames(supabase, jpegFrames, dealId, userId);
        } catch (uploadErr: unknown) {
          console.error("[runAnalyze] Frame upload failed:", uploadErr);
          persistErrors.push(
            "Analysis complete, but frames could not be saved. Findings will not persist across tab switches for this analysis.",
          );
